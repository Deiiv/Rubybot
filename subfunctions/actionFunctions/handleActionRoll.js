...
var handleActionRoll = function (msg) {
	...
	if (messageContent[1]) {
		...
		if (isNaN(x) || x < minX || x > maxX) {
			let message = new Discord.MessageEmbed().setColor(process.env.embedColour).addField("X in !roll XdY must be between " + minX + " and " + maxX, ":game_die:");
			msg.channel.send(message);
		} else if (isNaN(y) || y < minY || y > maxY) {
			let message = new Discord.MessageEmbed().setColor(process.env.embedColour).addField("Y in !roll XdY must be between " + minY + " and " + maxY, ":game_die:");
			msg.channel.send(message);
		} else {
			for (let i = 0; i < x; i++) {
				numbers.push(Math.floor(Math.random() * y) + 1).toString();
			}
			let message = new Discord.MessageEmbed().setColor(process.env.embedColour).addField(":game_die: " + messageContent[1], numbers.join(" | "));
			msg.channel.send(message);
		}
	}
};
