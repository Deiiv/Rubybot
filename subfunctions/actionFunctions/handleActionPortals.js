	...
	getSiteData("dofus-portals.fr", `/portails/${server.code}`)
		.then((siteData) => {
			...
			portalList.forEach((portal, i) => {
				...
				let message = new EmbedBuilder().setColor(process.env.embedColour).setTitle(`${dimensionName} Portal Info - ${server.name}`).setDescription(text).setThumbnail(thumbnail);
