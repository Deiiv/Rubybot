...
var handleActionView = function (msg) {
	...
	//view user by IGN !view user IGN
		...
		if (messageContent[2]) {
			...
			let message = new EmbedBuilder().setColor(process.env.embedColour).setTitle("Invalid input!").setDescription("View proper usage by calling !help prof");
			msg.channel.send({ embeds: [message] });
		}
	}
	//view profs !view prof OR !view prof level
		...
		if (messageContent[1]) {
			...
			if (profList.includes(prof)) {
				...
				let message = new EmbedBuilder().setColor(process.env.embedColour).setTitle("Invalid profession! List of valid professions:").setDescription(profList.toString());
				msg.channel.send({ embeds: [message] });
			}
		} else {
			let message = new EmbedBuilder().setColor(process.env.embedColour).setTitle("Invalid input!").setDescription("View proper usage by calling !help prof");
