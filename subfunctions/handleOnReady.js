...
var handleOnReady = function (client) {
	...
	client.guilds.cache.forEach(function (guild) {
		...
		if (!process.env.pepeRuby)
			process.env.pepeRuby = guild.emojis.cache.find((emoji) => emoji.name === "pepeRuby") || "";
		if (!process.env.monkaThink)
			process.env.monkaThink = guild.emojis.cache.find((emoji) => emoji.name === "monkaThink") || "";
		if (!process.env.pepeCry) process.env.pepeCry = guild.emojis.cache.find((emoji) => emoji.name === "pepeCry") || "";
		if (!process.env.peepoHappy)
			process.env.peepoHappy = guild.emojis.cache.find((emoji) => emoji.name === "peepoHappy") || "";
		if (!process.env.pepoG) process.env.pepoG = guild.emojis.cache.find((emoji) => emoji.name === "pepoG") || "";
		if (!process.env.pogpeach)
			process.env.pogpeach = guild.emojis.cache.find((emoji) => emoji.name === "pogpeach") || "";
		...
		if (!process.env.phoebepog)
			process.env.phoebepog = guild.emojis.cache.find((emoji) => emoji.name === "PhoebePog") || "";
		if (!process.env.phoepeek)
			process.env.phoepeek = guild.emojis.cache.find((emoji) => emoji.name === "PhoePeek") || "";
